...
class DebtDetailBloc extends Bloc<DebtDetailEvent, DebtDetailState> {
  DebtDetailBloc({required DebtRepository debtRepository})
    ...
    on<DebtDetailDeleteRequested>(_onDelete);
  }
  ...
  Future<void> _onRefresh(
    DebtDetailRefreshRequested event,
    Emitter<DebtDetailState> emit,
  ) async {
    await _fetch(emit);
  }
  ...
  Future<void> _onDelete(
    DebtDetailDeleteRequested event,
    Emitter<DebtDetailState> emit,
  ) async {
    final current = state;
    try {
      await _repo.delete(_debt!.id);
      emit(DebtDetailDeleted());
    } catch (e) {
      emit(DebtDetailError(_parseError(e)));
      if (current is DebtDetailLoaded) emit(current);
    }
  }
